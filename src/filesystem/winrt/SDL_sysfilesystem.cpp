...
extern "C" char *SDL_SYS_GetExeName(void)
{
    char *path = WIN_GetModulePath(NULL);
    if (!path) {
        return NULL;
    }
    ...
    const size_t slen = SDL_strlen(ptr);
    SDL_memmove(path, ptr + 1, slen);
    ptr = (char *) SDL_realloc(path, slen);
    return ptr ? ptr : path;
}
