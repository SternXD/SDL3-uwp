...
static HRESULT D3D11_CreateSwapChain(SDL_Renderer *renderer, int w, int h)
{
    ...
    if (coreWindow) {
        ...
        if (!panel) {
            SDL_SetError(SDL_COMPOSE_ERROR("XAML SwapChainBackgroundPanel was not set on the window"));
            result = E_FAIL;
            goto done;
        }

        result = ISwapChainBackgroundPanelNative_SetSwapChain(panel, (IDXGISwapChain *)data->swapChain);
        if (FAILED(result)) {
            WIN_SetErrorFromHRESULT(SDL_COMPOSE_ERROR("ISwapChainBackgroundPanelNative::SetSwapChain"), result);
            goto done;
        }
#else
        SDL_SetError(SDL_COMPOSE_ERROR("XAML support is not available on this platform"));
        result = E_FAIL;
        goto done;
#endif
    } else {
